...
BOUTIQUE (
    ...
    email,  not null
    logo,
    id_statut_boutique FK,       not null     -- actif / suspendu (change) fk 
    id_centre FK  not null --(considering one boutique is in one centre) ok
...
Annonce(
    id_annonce
    taget  not null --public (tlmd), boutique et proprio
...
DETAIL_Pannier (
    ...
    shipping fee -- 0 if sur place and set price eo if not (jerevana) look at status in panier
